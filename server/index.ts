...
(async () => {
  ...
  // Initialize discovery for all active OLTs before starting the server
  try {
    log('Initializing ONU discovery system...');
    await discoveryManager.initializeAllActiveOlts();
  } catch (error: any) {
    log(`Warning: Discovery initialization failed: ${error.message}`);
    log('Server will continue but discovery may not be running');
  }
