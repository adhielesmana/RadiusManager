#!/bin/bash
# Quick Fix for nginx SSL Certificate Issue (v3)
# Uses tee to overwrite file in-place (avoids "Resource busy")
...
echo -e "${BLUE}================================================${NC}"
echo -e "${BLUE}Quick Fix: nginx SSL Certificate Issue (v3)${NC}"
echo -e "${BLUE}================================================${NC}"
echo ""
...
grep -n "ssl_certificate" "$TEMP_DIR/nginx.conf" | grep -v "^[[:space:]]*#" || echo "  (none found - already fixed?)"
echo ""
...
# Apply modified config using tee (overwrites in-place, no mv needed)
echo -e "${BLUE}8. Applying modified nginx.conf (using tee)...${NC}"
cat "$TEMP_DIR/nginx.conf" | docker exec -i "$NGINX_CONTAINER" tee /etc/nginx/nginx.conf >/dev/null 2>&1 || {
...
if docker exec "$NGINX_CONTAINER" nginx -t 2>&1 | grep -q "successful"; then
    ...
    # Restore backup using tee
    docker exec "$NGINX_CONTAINER" sh -c 'if [ -f /etc/nginx/nginx.conf.backup ]; then cat /etc/nginx/nginx.conf.backup | tee /etc/nginx/nginx.conf >/dev/null; fi' 2>/dev/null || {
