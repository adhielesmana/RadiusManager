...
# Fixes "SSL: no alternative certificate subject name matches" error
# by commenting out global SSL directives in nginx.conf
...
echo -e "${BLUE}================================================${NC}"
echo -e "${BLUE}Quick Fix: nginx SSL Certificate Issue${NC}"
echo -e "${BLUE}================================================${NC}"
echo ""
...
# Stop nginx (|| true to avoid abort if already stopped)
echo -e "${BLUE}7. Stopping nginx to apply changes...${NC}"
docker stop "$NGINX_CONTAINER" >/dev/null 2>&1 || true
echo -e "${GREEN}✓ Nginx stopped${NC}"
echo ""

# Copy back
echo -e "${BLUE}8. Copying modified nginx.conf back to container...${NC}"
docker cp "$TEMP_DIR/nginx.conf" "$NGINX_CONTAINER":/etc/nginx/nginx.conf 2>/dev/null || {
    echo -e "${RED}✗ Could not copy nginx.conf back${NC}"
...
echo -e "${GREEN}✓ Modified nginx.conf copied back${NC}"

# Also copy backup to container for rollback if needed
...
# Test config
echo -e "${BLUE}10. Testing nginx configuration...${NC}"
...
# Reload nginx
echo -e "${BLUE}11. Reloading nginx...${NC}"
...
# Test SSL
echo -e "${BLUE}12. Testing SSL certificate...${NC}"
echo ""
